...
class G3DLayer(MessagePassing, LoggingMixin):
    """The G3D layer as described in [M-OFDFT]_.
    ...
    def forward(
        self, x: Tensor, edge_index: Adj, batch: Tensor, edge_attr: OptTensor = None, length=None
    ) -> Tensor:
        ...
        # Attention Block
        if isinstance(self.norm_1, LayerNorm):
        ...
        query = query.contiguous().view(-1, self.heads, self.channels_per_head)
        key = key.contiguous().view(-1, self.heads, self.channels_per_head)
        value = value.contiguous().view(-1, self.heads, self.channels_per_head)
        ...
        # MLP Block
        if isinstance(self.norm_2, LayerNorm):
...
class G3DLayerMulSilu(G3DLayer):
    """G3D layer with SiLU activation function."""
